...
class LBMCUDAUpgrade:
    ...
    def patch(sim):
        ...
        if collision_mode in ('mrt', 'mrt_smag'):
        ...
        # -- Smagorinsky: pre-compute (Cs * dx)^2 in lattice units ---------
        if collision_mode == 'mrt_smag':
            Cs = float(getattr(cfg, 'smag_Cs', 0.1))
            sim._cuda_Cs_dx2 = float(Cs ** 2)   # dx=1 in lattice units
            sim._cuda_omega0  = float(sim.omega_plus)  # base viscous rate
        else:
            sim._cuda_Cs_dx2 = None
            sim._cuda_omega0  = None

        # -- 3. Prepare f_post buffer -----------------------------------------
        if not hasattr(sim, '_f_post') or sim._f_post.shape != sim.f.shape:
            sim._f_post = torch.empty_like(sim.f)

        # _f_stream removed: pull streaming now writes f_post -> f directly.
        # Saves ~1.46 GB at 19M cells (one full 19xN float32 buffer).
        sim._f_stream = None
        ...
        flow_dir = cfg.flow_dir.upper().strip()
        _axis    = {'X': 0, 'Y': 1, 'Z': 2}[flow_dir[1]]
        _roles   = resolve_face_roles(cfg)
        _non_periodic = set()
        for _face, _role in _roles.items():
            if _role in ('inlet', 'outlet', 'wall'):
                _non_periodic.add({'X': 0, 'Y': 1, 'Z': 2}[_face[1]])
        _pbc = [d not in _non_periodic for d in range(3)]
        sim._cuda_periodic = tuple(_pbc)
        ...
        if collision_mode == "mrt_smag":
            Cs = float(getattr(cfg, "smag_Cs", 0.1))
            print(f"  collision       = mrt_smag  (Cs={Cs:.3f}, Cs_dx2={Cs**2:.4f})")
        else:
    ...
    def _make_collide(collision_mode: str):
        ...
        def _collide_mrt_smag(sim_self):
            N = sim_self.nx * sim_self.ny * sim_self.nz
            f_flat     = sim_self.f.reshape(19, N).contiguous()
            fpost_flat = sim_self._f_post.reshape(19, N)

            LBM_CUDA.macro_collide_mrt_smag(
                f_flat, fpost_flat,
                sim_self.rho.reshape(N),
                sim_self.u.reshape(N),
                sim_self.v.reshape(N),
                ...
                sim_self._solid_u8.reshape(N),
                sim_self._S_cuda,
                float(sim_self._cuda_Cs_dx2),
                float(sim_self._cuda_omega0))

            sim_self.u[sim_self.solid_flow]  = 0.
            sim_self.v[sim_self.solid_flow]  = 0.
            sim_self.wv[sim_self.solid_flow] = 0.

        dispatch = {'bgk': _collide_bgk, 'trt': _collide_trt,
                    'mrt': _collide_mrt, 'mrt_smag': _collide_mrt_smag}
    ...
    def _stream_cuda(sim_self):
        ...
        # Pull scheme: read f_post (post-collision), write directly into f.
        # No aliasing: pull reads neighbors from f_post, writes to f (different tensors).
        # _f_stream buffer eliminated -- saves 1.46 GB at 19M cells.
        LBM_CUDA.stream_pull(
            sim_self._f_post.contiguous(),
            sim_self.f,
            sim_self.nx, sim_self.ny, sim_self.nz,
            int(px), int(py), int(pz))
        # f now holds streamed populations; f_post still valid for IBB
...
#     if getattr(sim_self.cfg, 'outlet_bc_mode', 'pressure').lower() == 'convective':
#         FlowSolverUpgrade._apply_flow_bcs_convective(sim_self)
#         return
...
def _apply_flow_bcs_cuda_OLD(sim_self):
    cfg  = sim_self.cfg
    fdir = cfg.flow_dir.upper().strip()
    sign = +1 if fdir[0] == '+' else -1
    axis = {'X': 0, 'Y': 1, 'Z': 2}[fdir[1]]
    ...
    inlet_idx  = 0 if sign == +1 else n_along - 1
    outlet_idx = n_along - 1 if sign == +1 else 0
    interior_idx = (outlet_idx - 1) if outlet_idx > 0 else 1

    n1, n2 = {0: (sim_self.ny, sim_self.nz),
              1: (sim_self.nx, sim_self.nz),
              2: (sim_self.nx, sim_self.ny)}[axis]
    ...
    # --- velocity inlet (always pressure/Zou-He) ---
    LBM_CUDA.zou_he_bc(
        f_flat, s_flat,
        sim_self.nx, sim_self.ny, sim_self.nz,
        axis, inlet_idx, interior_idx,
        0,                              # bc_type=0: velocity
        u_signed, float(cfg.rho_out_lat), sign)

    # --- outlet: pressure or convective ---
    outlet_mode = getattr(cfg, 'outlet_bc_mode', 'pressure').lower()

    if outlet_mode == 'convective':
        # unknown directions: pointing INTO domain at outlet face
        bc_at_low   = (outlet_idx == 0)
        unknown_cax = +1 if bc_at_low else -1

        LBM_CUDA.convective_bc(
            f_flat, s_flat,
            sim_self.nx, sim_self.ny, sim_self.nz,
            axis, outlet_idx, interior_idx,
            unknown_cax, float(cfg.rho_out_lat), n1, n2)
...
def _apply_flow_bcs_cuda(sim_self):
    ...
    u_max       = float(getattr(cfg, 'u_lat_max', 0.5))
    rho_out     = float(cfg.rho_out_lat)
    fdir        = cfg.flow_dir.upper().strip()
    flow_sign   = +1 if fdir[0] == '+' else -1
    flow_axis   = {'X': 0, 'Y': 1, 'Z': 2}[fdir[1]]
    u_signed    = float(max(-u_max, min(u_max, float(sim_self.u_in_lat) * flow_sign)))
    outlet_mode = getattr(cfg, 'outlet_bc_mode', 'pressure').lower()

    plane_dims = {
        0: (sim_self.ny, sim_self.nz),
        1: (sim_self.nx, sim_self.nz),
        2: (sim_self.nx, sim_self.ny),
    }

    roles = resolve_face_roles(cfg)
    ...
    for face, role in roles.items():
        ...
        n_along = (sim_self.nx, sim_self.ny, sim_self.nz)[ax]
        fsign   = +1 if face[0] == '+' else -1

        plane_idx    = n_along - 1 if fsign == +1 else 0
        interior_idx = (plane_idx - 1) if plane_idx > 0 else 1
        n1, n2       = plane_dims[ax]
        ...
        if role == 'inlet':
            LBM_CUDA.zou_he_bc(
                f_flat, s_flat,
                sim_self.nx, sim_self.ny, sim_self.nz,
                ax, plane_idx, interior_idx,
                0,
                u_signed, rho_out, flow_sign)

        else:  # outlet
            is_primary = (ax == flow_axis)

            if outlet_mode == 'convective' and is_primary:
